        ...
        <div className="p-6 bg-gradient-to-b from-white/5 to-transparent">
          <div className="flex items-end justify-center gap-4">
            ...
            <div className="flex flex-col items-center">
              <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${getAvatarColor(entries[1].student_name)} flex items-center justify-center mb-2 ring-4 ring-gray-400/50`}>
                <span className="text-white font-bold text-lg">{getInitials(entries[1].student_name)}</span>
              </div>
              <Medal className="w-6 h-6 text-gray-300 -mt-1 mb-1" />
              <p className="text-white font-semibold text-sm text-center max-w-[100px] truncate">{entries[1].student_name}</p>
              <p className="text-amber-400 font-bold">{entries[1].total_points.toLocaleString()}</p>
              <div className="w-24 h-20 bg-gradient-to-t from-gray-500/30 to-gray-400/20 rounded-t-lg mt-2" />
            </div>

            {/* First Place */}
            <div className="flex flex-col items-center -mb-4">
              <div className={`w-20 h-20 rounded-full bg-gradient-to-br ${getAvatarColor(entries[0].student_name)} flex items-center justify-center mb-2 ring-4 ring-yellow-400/50 shadow-lg shadow-yellow-500/25`}>
                <span className="text-white font-bold text-xl">{getInitials(entries[0].student_name)}</span>
              </div>
              <Crown className="w-8 h-8 text-yellow-400 -mt-1 mb-1" />
              <p className="text-white font-bold text-center max-w-[120px] truncate">{entries[0].student_name}</p>
              <p className="text-amber-400 font-bold text-lg">{entries[0].total_points.toLocaleString()}</p>
              <div className="w-28 h-28 bg-gradient-to-t from-yellow-500/30 to-amber-400/20 rounded-t-lg mt-2" />
            </div>

            {/* Third Place */}
            <div className="flex flex-col items-center">
              <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${getAvatarColor(entries[2].student_name)} flex items-center justify-center mb-2 ring-4 ring-amber-600/50`}>
                <span className="text-white font-bold">{getInitials(entries[2].student_name)}</span>
              </div>
              <Medal className="w-5 h-5 text-amber-600 -mt-1 mb-1" />
              <p className="text-white font-medium text-sm text-center max-w-[90px] truncate">{entries[2].student_name}</p>
              <p className="text-amber-400 font-bold text-sm">{entries[2].total_points.toLocaleString()}</p>
              <div className="w-20 h-14 bg-gradient-to-t from-amber-700/30 to-amber-600/20 rounded-t-lg mt-2" />
            </div>
          </div>
        </div>
      )}
            ...
            {entries.map((entry, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getAvatarColor(entry.student_name)} flex items-center justify-center`}>
                    <span className="text-white font-bold">{getInitials(entry.student_name)}</span>
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`font-bold ${isCurrentUser ? 'text-violet-300' : 'text-white'}`}>
                        {entry.student_name}
                      </p>
                      {isCurrentUser && (
                        <span className="px-2 py-0.5 bg-violet-500/30 text-violet-300 text-xs rounded-full font-medium">You</span>
                      )}
                      {rank <= 3 && (
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold text-amber-400">{entry.total_points.toLocaleString()}</p>
